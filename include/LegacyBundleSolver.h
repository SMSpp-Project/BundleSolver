/*--------------------------------------------------------------------------*/
/*-------------------- File LegacyBundleSolver.h ---------------------------*/
/*--------------------------------------------------------------------------*/
/** @file
 * Header file for the LegacyBundleSolver class, which implements the Solver
 ...
 * \endlink
 *
 * In particular, LegacyBundleSolver implements the Incremental version of the
 ...
 * \endlink
 *
 * LegacyBundleSolver is capable of solving any Block such that:
 *
...
{
 class LegacyBundleSolverState;  // forward declaration of LegacyBundleSolverState

/*--------------------------------------------------------------------------*/
/*------------------------------- CLASSES ----------------------------------*/
/*--------------------------------------------------------------------------*/
/** @defgroup LagBFunction_CLASSES Classes in LegacyBundleSolver.h
 *  @{ */

/*--------------------------------------------------------------------------*/
/*-------------------------- CLASS LegacyBundleSolver ----------------------------*/
/*--------------------------------------------------------------------------*/
/*--------------------------- GENERAL NOTES --------------------------------*/
/*--------------------------------------------------------------------------*/
/// A CDASolver using a (Generalized) Bundle algorithm
/** LegacyBundleSolver implements the Solver interface, in particular in its
...
class LegacyBundleSolver : public CDASolver {
...
/*--------------------------------------------------------------------------*/
  ...
  * that LegacyBundleSolver has in addition to these of CDASolver. The value
 ...
 intBPar7 ,  ///< how well-behaved LegacyBundleSolver is w.r.t. other Solver
...
/*--------------------------------------------------------------------------*/
  ...
  * type that LegacyBundleSolver has in addition to these of CDASolver. The value
...
/*--------------------------------------------------------------------------*/
  ...
  * vector-of-int type that LegacyBundleSolver has in addition to these of
...
/** @} ---------------------------------------------------------------------*/
/*----------------- CONSTRUCTING AND DESTRUCTING LegacyBundleSolver --------------*/
/*--------------------------------------------------------------------------*/
/** @name Constructing and destructing LegacyBundleSolver
 *  @{ */

 /// constructor: ensure every field is initialized

 LegacyBundleSolver( void ) : CDASolver() , Result( kUnEval ) , NumVar( 0 ) ,
...
/*--------------------------------------------------------------------------*/
 /// destructor: cleanly detaches the LegacyBundleSolver from the Block

 virtual ~LegacyBundleSolver() { set_Block( nullptr ); }
 ...
 *  @{ */
 ...
 /** Gives the LegacyBundleSolver access to the Block it has to solve; note that
  * this does not register the LegacyBundleSolver among the Solver of the Block,
  ...
  * LegacyBundleSolver, and all the nontrivial internal data structures of
  * LegacyBundleSolver are set up.
  *
  * If \p block == nullptr, the LegacyBundleSolver is completely cleaned up and
...
/*--------------------------------------------------------------------------*/
 /// set the int parameters of LegacyBundleSolver
 /** Set the int parameters specific of LegacyBundleSolver, together with the
  * parameters of CDASolver that LegacyBundleSolver actually "listens to":
  *
  ...
  *                    events
  *
  * - intLogVerb [0]: "verbosity" of the LegacyBundleSolver log
  *                   0 = no log
  ...
  *   LegacyBundleSolver to perform aggregation at every iteration (for every
  ...
  *   other items, although the LegacyBundleSolver will complain and stop if less
  *   than BPar4 are given. In LegacyBundleSolver, the number
  *
  ...
  *
  * - intBPar7 [2]: This parameter, coded bit-wise, controls if LegacyBundleSolver
  ...
  *   pool(s) of the C05Function(s). Hence, what the LegacyBundleSolver does to the
  *   global pool may have an impact on the other Solver, if any. This
  *   parameter controls whether LegacyBundleSolver tries as hard as possible to
  ...
  *   the global pool as its exclusive property". To do so, LegacyBundleSolver
  *   handles the slot of the global pool in different ways according to the
  *   value in the first two bits of intBPar7 ( intBPar7 & 3 ):
  *
  *   = 0 means that LegacyBundleSolver will never override any position in the
  ...
  *     LegacyBundleSolver always "proceeds from left to right", i.e., selects the
  ...
  *
  *   = 1 means that LegacyBundleSolver will not immediately delete from the global
  *     pool a linearization that it removes from the bundle (the master
  *     problem). While the linearization is kept there, LegacyBundleSolver
  ...
  *     actually overwritten by LegacyBundleSolver, thereby leaving it available
  *     to other Solver.
  *
  *   = 2 means that LegacyBundleSolver will immediately delete from the global
  *     pool any linearization that it removes from the bundle (the master
  *     problem). This makes sense if LegacyBundleSolver is the only Solver
  ...
  *
  *   = 3 means that LegacyBundleSolver will immediately delete from the global
  *     pool any linearization that it removes from the bundle (the master
  ...
  *
  *   The bit 2 ( intBPar7 & 4 ) rather decides how LegacyBundleSolver reacts to
  *   Modification telling that some other Solver have generated a new
  *   linearization. If the bit is 0, then LegacyBundleSolver plainly ignores it,
  *   which is likely the best strategy if producing linearizations is
  *   "cheap". However, if ( intBPar7 & 3 ) < 3 LegacyBundleSolver does take note
  *   that a linearization is there in order to avoid to touch it "unless
  *   strictly necessary". If the bit is 1 instead, then LegacyBundleSolver will
  ...
  *   phase: if it is == 1, then LegacyBundleSolver will also scan the global pool
  ...
  *                  LegacyBundleSolver uses the "easy components" approach on
  ...
  *   Note, however, that in this case LegacyBundleSolver will require a functioning
  ...
  *   Furthermore, the following three bits, if 1, instruct LegacyBundleSolver to
  ...
  *   changed during the course of the optimization and the LegacyBundleSolver to
  ...
  *   capability of LegacyBundleSolver to produce global valid lower bounds (for a
  ...
  *            by get_var_solution(). Some approaches using LegacyBundleSolver may
  ...
  *            LegacyBundleSolverState [see] are not trusted when it is put() back
  *            in LegacyBundleSolver. This causes the re-computation of all
  ...
  *     to which LegacyBundleSolver is registered is not the same as that when the
  *     LegacyBundleSolverState was get(), which may happen either if they are
  *     actually two different (similar, but not identical) Block, or if
  *     LegacyBundleSolver was detached, some changes were effected in the Block
  *     and then LegacyBundleSolver was re-attached. this way of using 
  *     [LegacyBundleSolver]State is explicitly permitted by the definition of
  *     State, with the provision that the Solver must be able to identify
  *     somehow the inconsistencies that it can create. since LegacyBundleSolver
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// set the double parameters of LegacyBundleSolver
 /** Set the double parameters specific of LegacyBundleSolver, together with the
  * parameters of CDASolver that LegacyBundleSolver actually "listens to":
  *
  ...
  *   on the ability of LegacyBundleSolver to declare globally valid lower bounds
  *   (for a minimization problem, upper bounds for a maximization one).
  *   Choosing a very small value for dblZNEps may result in LegacyBundleSolver not
  ...
  *   at the same time, in which case the LegacyBundleSolver will typically favor
  *   the SS. 
  *
  ...
  *                 of t can clearly help the LegacyBundleSolver to perform
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// move the string parameters of LegacyBundleSolver
 /** Move in the string parameters specific of LegacyBundleSolver, together with
  * the parameters of CDASolver that LegacyBundleSolver actually "listens to":
  *
  ...
  *   in which the LegacyBundleSolver is registered to the Block; if strEasyCfg is
  ...
  *   to set_ComputeConfig) at the time in which the LegacyBundleSolver is
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// move in the vector-of-int parameters of LegacyBundleSolver
 /** Move in the given vector-of-int parameters specific of LegacyBundleSolver,
  * together with the parameters of CDASolver that LegacyBundleSolver actually
  * "listens to":
  *
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// move in the vector-of-string parameters of LegacyBundleSolver
 /** Move in the given vector-of-string parameters specific of LegacyBundleSolver,
  * together with the parameters of CDASolver that LegacyBundleSolver actually
  * "listens to":
  *
  ...
  *   when LegacyBundleSolver is register()-ed to the Block. This parameter works
  ...
  *   C05Function when LegacyBundleSolver is register()-ed to the Block. This
...
/*--------------------------------------------------------------------------*/
 /// set the ostream for the LegacyBundleSolver log
 ...
 * LegacyBundleSolver some relevant data about the Block it is solving.
 *  @{ */
  ...
  * the Block that the LegacyBundleSolver is solving. This method should not be
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  * makes up the objective of the Block that the LegacyBundleSolver is solving. 
 ...
 *
 *  LegacyBundleSolver heeds to all three "basic" types of events:
 *
...
/*--------------------------------------------------------------------------*/
 /// LegacyBundleSolver always returns a primal solution, possibly unfeasible
 
 bool has_var_solution( void ) override { return( true ); }

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// LegacyBundleSolver always returns a dual solution, possibly unfeasible
 /** LegacyBundleSolver always returns a dual solution, possibly unfeasible. This
...
/*--------------------------------------------------------------------------*/
  ...
  *   LegacyBundleSolver is using is actually the dual optimal solution of the
...
/*--------------------------------------------------------------------------*/
  ...
  *   LegacyBundleSolver is solving is actually the primal optimal solution of
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *  standard stopping condition of LegacyBundleSolver roughly speaking reads
  *
...
/*--------------------------------------------------------------------------*/
/** @name Handling the parameters of the LegacyBundleSolver
 *
 *  @{ */
...
/*--------------------------------------------------------------------------*/
 ...
 [[nodiscard]] idx_type int_par_str2idx( const std::string & name )
  ...
  static const std::map< std::string , idx_type > int_pars_map = {
   { "intBPar1" , LegacyBundleSolver::intBPar1  } ,
   { "intBPar2" , LegacyBundleSolver::intBPar2  } ,
   { "intBPar3" , LegacyBundleSolver::intBPar3 } ,
   { "intBPar4" , LegacyBundleSolver::intBPar4 } ,
   { "intBPar6" , LegacyBundleSolver::intBPar6 } ,
   { "intBPar7" , LegacyBundleSolver::intBPar7 } ,
   { "intMnSSC" , LegacyBundleSolver::intMnSSC } ,
   { "intMnNSC" , LegacyBundleSolver::intMnNSC } ,
   { "inttSPar1" , LegacyBundleSolver::inttSPar1 } ,
   { "intMaxNrEvls" , LegacyBundleSolver::intMaxNrEvls } ,
   { "intDoEasy" , LegacyBundleSolver::intDoEasy } ,
   { "intWZNorm" , LegacyBundleSolver::intWZNorm } ,
   { "intFrcLstSS" , LegacyBundleSolver::intFrcLstSS } ,
   { "intTrgtMng" , LegacyBundleSolver::intTrgtMng } ,
   { "intMPName" , LegacyBundleSolver::intMPName } ,
   { "intMPlvl" , LegacyBundleSolver::intMPlvl } ,
   { "intQPmp1" , LegacyBundleSolver::intQPmp1 } ,
   { "intQPmp2" , LegacyBundleSolver::intQPmp2 } ,
   { "intOSImp1" , LegacyBundleSolver::intOSImp1 } ,
   { "intOSImp2" , LegacyBundleSolver::intOSImp2 } ,
   { "intOSImp3" , LegacyBundleSolver::intOSImp3 } ,
   { "intRstAlg" , LegacyBundleSolver::intRstAlg } ,
   };
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 [[nodiscard]] idx_type dbl_par_str2idx( const std::string & name )
  ...
  static const std::map< std::string , idx_type > dbl_pars_map = {
   { "dblNZEps" , LegacyBundleSolver::dblNZEps } ,
   { "dbltStar" , LegacyBundleSolver::dbltStar } ,
   { "dblMinNrEvls" , LegacyBundleSolver::dblMinNrEvls } ,
   { "dblBPar5" , LegacyBundleSolver::dblBPar5 } ,
   { "dblm1" , LegacyBundleSolver::dblm1 } ,
   { "dblm2" , LegacyBundleSolver::dblm2 } ,
   { "dblm3" , LegacyBundleSolver::dblm3 } ,
   { "dblmxIncr" , LegacyBundleSolver::dblmxIncr } ,
   { "dblmnIncr" , LegacyBundleSolver::dblmnIncr } ,
   { "dblmxDecr" , LegacyBundleSolver::dblmxDecr } ,
   { "dblmnDecr" , LegacyBundleSolver::dblmnDecr } ,
   { "dbltMaior" , LegacyBundleSolver::dbltMaior } ,
   { "dbltMinor" , LegacyBundleSolver::dbltMinor } ,
   { "dbltInit" , LegacyBundleSolver::dbltInit } ,
   { "dbltSPar2" , LegacyBundleSolver::dbltSPar2 } ,
   { "dbltSPar3" , LegacyBundleSolver::dbltSPar3 } ,
   { "dblCtOff" , LegacyBundleSolver::dblCtOff }
   };
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 [[nodiscard]] idx_type vstr_par_str2idx( const std::string & name )
  ...
  static const std::map< std::string , idx_type > vstr_pars_map = {
   { "vstrCmpCfg" , LegacyBundleSolver::vstrCmpCfg } ,
   { "vstr_C05_SPAR_Names" , LegacyBundleSolver::vstr_C05_SPAR_Names } ,
   { "vstr_C05_SPAR_Vals" , LegacyBundleSolver::vstr_C05_SPAR_Vals } ,
   { "vstr_C05_EI_SPAR_Names" , LegacyBundleSolver::vstr_C05_EI_SPAR_Names } ,
   { "vstr_C05_EI_SPAR_Vals" , LegacyBundleSolver::vstr_C05_EI_SPAR_Vals }
   };
...
/*--------------------------------------------------------------------------*/
/** @name Handling the State of the LegacyBundleSolver
 *  @{ */

 State * get_State( void ) const override;

/*--------------------------------------------------------------------------*/
 /// sets the current "internal state" of the LegacyBundleSolver
 /** This method reads \p state, which must be a LegacyBundleSolverState (otherwise
  * exception is thrown) and uses it to set the  "internal state" of the
  * LegacyBundleSolver. \p state is not changed (could not, it's const) so that it
...
/*--------------------------------------------------------------------------*/
 ...
 /** This method reads \p state, which must be a LegacyBundleSolverState (otherwise
  * exception is thrown) and uses it to set the  "internal state" of the
  * LegacyBundleSolver. \p state is "moved inside" the LegacyBundleSolver, hence at the
...
/*--------------------------------------------------------------------------*/
 ...
 friend class LegacyBundleSolverState;  // make LegacyBundleSolverState friend
 ...
 * strictly necessary to use a MPSolver inside LegacyBundleSolver. This hack will
...
/*--------------------------------------------------------------------------*/
/*---------------------------- CONSTRUCTOR ---------------------------------*/
/** Constructor of the class: takes the pointer to the LegacyBundleSolver it has
 * to "serve". */

 FakeFiOracle( LegacyBundleSolver *solver ) : FiOracle() {
  bslv = solver;
  }
...
/*-------------------- PROTECTED PART OF THE CLASS -------------------------*/
 ...
 LegacyBundleSolver * bslv;  ///< the LegacyBundleSolver that I "serve"
...
/*--------------------------------------------------------------------------*/
 ...
 void guts_of_put_State( const LegacyBundleSolverState & state );
...
/*--------------------------------------------------------------------------*/
 ...
 int BPar7;         ///< if LegacyBundleSolver "plays nice" with other Solver
 ...
 *         separately, as it has not inside InnerLoop(). */
  ...
  * pool of component k where LegacyBundleSolver can put a new linearization when
 ...
 //
 // Owned by LegacyBundleSolver (not by the C05Function), in line with the
...
/*--------------------------------------------------------------------------*/
 ...
 };  // end( class LegacyBundleSolver )

/*--------------------------------------------------------------------------*/
/*--------------------- CLASS LegacyBundleSolverState ----------------------*/
/*--------------------------------------------------------------------------*/
/// class to describe the "internal state" of a LegacyBundleSolver
/** Derived class from State to describe the "internal state" of a
 * LegacyBundleSolver: the current stability centre, the proximal parameters, and
...
class LegacyBundleSolverState : public State {
...
/*----------------------------- PUBLIC TYPES -------------------------------*/
 ...
 using Index = LegacyBundleSolver::Index;
 using VarValue = LegacyBundleSolver::VarValue;
 using Vec_VarValue = LegacyBundleSolver::Vec_VarValue;

/*---------- CONSTRUCTING AND DESTRUCTING LegacyBundleSolverState ----------*/

 /// constructor, doing everything or nothing.
 /** Constructor of LegacyBundleSolverState. If provided with a pointer to a
  * LegacyBundleSolver it immediately copies its "internal state", which is the only
  * way in which the LegacyBundleSolverState can be initialised out of an existing
  * LegacyBundleSolverState. If nullptr is passed (as by default), then an "empty"
  * LegacyBundleSolverState is constructed that can only be filled by calling
  * deserialize(). */

 LegacyBundleSolverState( const LegacyBundleSolver * bs = nullptr ) : State() {
  if( ! bs ) {
   NrFi = NumVar = 0;
   return;
   }
  ...
  for( LegacyBundleSolver::Index i = 0 ; i < NrFi ; ++i ) {
...
/*--------------------------------------------------------------------------*/
 /// de-serialize a LegacyBundleSolverState out of netCDF::NcGroup
 /** De-serialize a LegacyBundleSolverState out of netCDF::NcGroup; see
  * LegacyBundleSolverState::serialize() for a description of the format. */
...
/*--------------------------------------------------------------------------*/
 ...
 virtual ~LegacyBundleSolverState() {
  for( auto el : v_comp_State )
   delete el;
  }

/*------ METHODS DESCRIBING THE BEHAVIOR OF A LegacyBundleSolverState ------*/

 /// serialize a LegacyBundleSolverState into a netCDF::NcGroup
 /** The method serializes the LegacyBundleSolverState into the provided
  * netCDF::NcGroup, so that it can later be read back by deserialize().
  *
  * After the call, \p group will contain:
  *
  * - The dimension "LegacyBundleSolver_NumVar" containing the (current) number
  *   of variables of all the components.
  *
  * - The variable "LegacyBundleSolver_Lambda", of type netCDF::NcDouble and
  *   indexed over the dimension LegacyBundleSolver_NumVar, which contains
  *   the current stability centre.
  *
  * - The scalar variable "LegacyBundleSolver_t", of type netCDF::NcDouble,
  *   which contains the current value of the (tremendous) t parameter.
  *
  * - The dimension "LegacyBundleSolver_NrFi" containing the total number of
  *   components (comprised the "easy" ones).
  *
  * - The dimension "LegacyBundleSolver_UpFiLmbdef" containing the number of
  ...
  *
  * - The dimension "LegacyBundleSolver_LwFiLmbdef" containing the number of
  *   components for which a non--INF lower bound value in Lambda is known.
  *   The dimension is optional, if it is not present then 0 is assumed.
  *
  * - The variable "LegacyBundleSolver_UpFiLmb", of type netCDF::NcDouble and
  *   indexed over the dimension LegacyBundleSolver_NrFi, which contains the
  *   known opper bound value on each component in Lambda. The variable
  *   must be there if "LegacyBundleSolver_UpFiLmbdef" (is defined and) has a
  *   value > 0, but it is ignored (and therefore is optional) otherwise.
  *
  * - The variable "LegacyBundleSolver_LwFiLmb", of type netCDF::NcDouble and
  *   indexed over the dimension LegacyBundleSolver_NrFi, which contains the
  *   lower opper bound value on each component in Lambda. The variable
  *   must be there if "LegacyBundleSolver_LwFiLmbdef" (is defined and) has a
  *   value > 0, but it is ignored (and therefore is optional) otherwise.
  *
  * - The variable "LegacyBundleSolver_Fi0Lmb", of type netCDF::NcDouble and
  ...
  *
  * - The variable "LegacyBundleSolver_global_LB", of type netCDF::NcDouble and
  *   indexed over no dimension (scalar), which contains the value of the
  *   algorithmic global LB. The variable is optional, if it is not present
  *   then - INF is assumed.
  *
  * - At most LegacyBundleSolver_NrFi netCDF::NcGroup with name
  *   "Component_State_X", with X an integer between 0 and
  *   LegacyBundleSolver_NrFi - 1, each one containing the State af the
...
/*-------------------------------- FRIENDS ---------------------------------*/
 ...
 friend class LegacyBundleSolver;  // make LegacyBundleSolver friend
...
/*-------------------------- PROTECTED METHODS -----------------------------*/
 ...
 void print( std::ostream &output ) const override {
  output << "LegacyBundleSolverState [" << this << "] with NrFi = " << NrFi
	 << " and NumVar = " << NumVar;
  }
...
/*--------------------------------------------------------------------------*/
 ...
 };  // end( class( LegacyBundleSolverState ) )

/** @} end( group( LegacyBundleSolver_CLASSES ) ) --------------------------*/
/*--------------------------------------------------------------------------*/
...
#endif  /* LegacyBundleSolver.h included */

/*--------------------------------------------------------------------------*/
/*---------------------- End File LegacyBundleSolver.h ---------------------*/
/*--------------------------------------------------------------------------*/
