/*--------------------------------------------------------------------------*/
/*--------------------- File LegacyBundleSolver.cpp ------------------------*/
/*--------------------------------------------------------------------------*/
/** @file
 * Implementation of the LegacyBundleSolver class.
 *
 ...
 *
 * LegacyBundleSolver is engineered to be "resistant" to Negative linearization 
 ...
 * and on std::cerr rather than on the LegacyBundleSolver log stream (if any), hence
...
static inline void pval( std::ostream & os , double val ) {
 if( val == LegacyBundleSolver::INFshift )
  os << "INF";
 else
  if( val == -LegacyBundleSolver::INFshift )
   os << "-INF";
...
static void Compact( LegacyBundleSolver::Vec_VarValue & g ,
		     LegacyBundleSolver::c_Subset & B )
{
...
static void set_difference_in_place( LegacyBundleSolver::Subset & S1 ,
				     LegacyBundleSolver::c_Subset & S2 )
{
...
static void set_union_in_place( LegacyBundleSolver::Subset & S1 ,
				LegacyBundleSolver::c_Subset & S2 )
{
...
static double norm( const LegacyBundleSolver::Vec_VarValue & v , char t )
{
 double res = 0;
 if( t == 0 ) {    // INF-norm
  for( auto el : v )
...
static void vect_sum( LegacyBundleSolver::Vec_VarValue & v1 , double * v2 )
{
 for( auto & el : v1 )
  el += *(v2++);
 }
...
// register LegacyBundleSolver to the Solver factory

SMSpp_insert_in_factory_cpp_0( LegacyBundleSolver );

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

// register LegacyBundleSolverState to the State factory

SMSpp_insert_in_factory_cpp_0( LegacyBundleSolverState );

/*--------------------------------------------------------------------------*/
/*-------------------- METHODS OF LegacyBundleSolver -----------------------*/
/*--------------------------------------------------------------------------*/

int LegacyBundleSolver::compute( bool changedvars )
{
 ...
 if( Result == kStillRunning )  // ... or from the same
  throw( std::logic_error( "LegacyBundleSolver::compute() called within itself" )
	 );
 ...
 if( ! f_Block ) {    // no Block is there to compute
  ...
  LegacyBundleSolver_error_return:
 ...
 if( MaxIter == 0 ) {  // no iteration must be performed
  Result = kStopIter;
  goto LegacyBundleSolver_error_return;
  }
 ...
 }  // end( LegacyBundleSolver::compute )

/*--------------------------------------------------------------------------*/
/*-------------------------- OTHER INITIALIZATIONS -------------------------*/
/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::set_Block( Block * block )
{
 ...
 if( ( ! owned ) && ( ! f_Block->lock( f_id ) ) )
  throw( std::runtime_error(
                       "LegacyBundleSolver: unable to lock the Block" ) );
 ...
 /* Two types of block can be handled by the LegacyBundleSolver:
 ...
 if( f_sparse_lambda ) {
  ...
  // this when f_sparse_lambda == true (see LegacyBundleSolver.cpp:8090+).
  ...
  // must be presented to LegacyBundleSolver in an order that is monotonic with
 ...
 if( DoEasy & 1 ) {
  ...
  if( ! NrEasy )
   ...
   if( NrEasy == NrFi )
    throw( std::logic_error(
	   "LegacyBundleSolver: all components are easy, this is no supported" ) );
 ...
 // this is necessary in that with BPar2 == 0, LegacyBundleSolver just takes
 ...
 // instead, LegacyBundleSolver ensures that the size of the global pool is *at
 // least* BPar2 by increasing it if it is below. this means that:
 // - LegacyBundleSolver never *decreases* the size of the global pool
 // - LegacyBundleSolver only uses the first BPar2 linearizations in each global
  ...
  Master->SetDim();  // clear all its internal state
   ...
   if( ( MPName & 1 ) && ( MPName & 16 ) ) {
    ...
    if( ! MILPMPCfg.empty() ) {
     ...
     if( ! bsc )
      throw( std::invalid_argument(
       "LegacyBundleSolver: strMPBSolverCfg does not deserialise to a "
 ...
 //
 // note that if ( BPar7 & 3 ) >= 2, LegacyBundleSolver will happily delete from
 ...
 // 
 // however, if ( BPar7 & 3 ) == 3 then LegacyBundleSolver does not care at all
 ...
 }  // end( LegacyBundleSolver::set_Block )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::set_par( idx_type par , int value )
{
 switch( par ) {
  case( intMaxIter ):
   if( value < 0 )
  ...
  case( intMPName ):
   ...
   // place of OSIMPSolver (combined with bit 0). See LegacyBundleSolver.h
 ...
 }  // end( LegacyBundleSolver::set_par( int ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::set_par( idx_type par , double value )
{
 switch( par ) {
  case( dblMaxTime ):
   if( value <= 0 )
 ...
 }  // end( LegacyBundleSolver::set_par( double ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::set_par( idx_type par , std::string && value )
{
 switch( par ) {
  case( strEasyCfg ):
 ...
 }  // end( LegacyBundleSolver::set_par( std::string && ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::set_par( idx_type par , std::vector< int > && value )
{
 if( par == vintNoEasy )
...
void LegacyBundleSolver::set_par( idx_type par ,
...
void LegacyBundleSolver::set_log( std::ostream * log_stream )
{
 f_log = log_stream;
 if( Master )
...
void LegacyBundleSolver::get_var_solution( Configuration *solc ) 
{
 ...
 }  // end( LegacyBundleSolver::get_var_solution )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::get_var_solution_easy_pi( Index k )
{
 if( ! ( ( DoEasy & 12 ) == 12 ) )
  throw( std::logic_error(
 ...
 }  // end( LegacyBundleSolver::get_var_solution_easy_pi() )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::get_var_solution_easy_rc( Index k )
{
 if( ! ( DoEasy & 8 ) )
...
void LegacyBundleSolver::get_dual_solution( Configuration * solc )
{
 ...
 }  // end( LegacyBundleSolver::get_dual_solution() )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::get_dual_solution_easy( Index k )
{
 auto nc = IsEasy[ k ]->get_numcols();
 ...
 }  // end( LegacyBundleSolver::get_dual_solution_easy() )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::get_dual_solution_hard( Index k )
{
 ...
 }  // end( LegacyBundleSolver::get_dual_solution_hard() )

/*--------------------------------------------------------------------------*/

int LegacyBundleSolver::get_int_par( idx_type par ) const
{
 switch( par ) {
 ...
 }  // end( LegacyBundleSolver::get_int_par )

/*--------------------------------------------------------------------------*/

double LegacyBundleSolver::get_dbl_par( idx_type par ) const
{
 switch( par ) {
 ...
 }  // end( LegacyBundleSolver::get_dbl_par )

/*--------------------------------------------------------------------------*/

const std::string & LegacyBundleSolver::get_str_par( idx_type par ) const
{
 switch( par ) {
 ...
 }  // end( LegacyBundleSolver::get_str_par )

/*--------------------------------------------------------------------------*/

const std::vector< int > & LegacyBundleSolver::get_vint_par( idx_type par ) const
{
 if( par == vintNoEasy )
  return( NoEasy );
 ...
 }  // end( LegacyBundleSolver::get_vint_par )

/*--------------------------------------------------------------------------*/

const std::vector< std::string > & LegacyBundleSolver::get_vstr_par( idx_type par )
 const
{
 switch( par ) {
...
State * LegacyBundleSolver::get_State( void ) const {
  return( new LegacyBundleSolverState( this ) );
  }  // end( LegacyBundleSolver::get_State )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::put_State( const State & state )
{
 // if state is not a LegacyBundleSolverState &, exception will be thrown
 const auto & s = dynamic_cast< const LegacyBundleSolverState & >( state );
 ...
 }  // end( LegacyBundleSolver::put_State( const & ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::put_State( State && state )
{
 // if state is not a LegacyBundleSolverState &&, exception will be thrown
 auto && s = dynamic_cast< LegacyBundleSolverState && >( state );
 ...
 }  // end( LegacyBundleSolver::put_State( && ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::serialize_State( netCDF::NcGroup & group ,
				    const std::string & sub_group_name ) const
{
 ...
 // do it "by hand" since there is no LegacyBundleSolverState available to call
 // State::serialize() from
 group.putAtt( "type", "LegacyBundleSolverState" );

 auto nv = group.addDim( "LegacyBundleSolver_NumVar" , NumVar );

 ( group.addVar( "LegacyBundleSolver_Lambda" , netCDF::NcDouble() , nv ) ).putVar(
			               { 0 } , {  NumVar } , Lambda.data() );

 ( group.addVar( "LegacyBundleSolver_t" , netCDF::NcDouble() ) ).putVar( &t );

 auto nfi = group.addDim( "LegacyBundleSolver_NrFi" , NrFi + 1 );

 if( UpFiLmbdef ) {
  group.addDim( "LegacyBundleSolver_UpFiLmbdef" , UpFiLmbdef );
  ( group.addVar( "LegacyBundleSolver_UpFiLmb" , netCDF::NcDouble() , nfi )
    ).putVar( { 0 } , { NrFi + 1 } , UpFiLmb.data() );
  }

 if( LwFiLmbdef ) {
  group.addDim( "LegacyBundleSolver_LwFiLmbdef" , LwFiLmbdef );
  ( group.addVar( "LegacyBundleSolver_LwFiLmb" , netCDF::NcDouble() , nfi )
    ).putVar( { 0 } , { NrFi + 1 } , LwFiLmb.data() );
  }

 if( Fi0Lmb != 0 )
  ( group.addVar( "LegacyBundleSolver_Fi0Lmb" , netCDF::NcDouble() )
    ).putVar( & Fi0Lmb );

 if( f_global_LB > - INFshift )
  ( group.addVar( "LegacyBundleSolver_global_LB" , netCDF::NcDouble() )
    ).putVar( & f_global_LB );
 ...
 }  // end( LegacyBundleSolver::serialize_State )

/*--------------------------------------------------------------------------*/
/*----------------------- OTHER PROTECTED METHODS --------------------------*/
/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::guts_of_put_State( const LegacyBundleSolverState & state )
{
 if( Result == kStillRunning )
  throw( std::logic_error(
	"LegacyBundleSolver::put_State() called within LegacyBundleSolver::compute()" ) );

 if( ( NrFi != state.NrFi ) || ( NumVar != state.NumVar ) )
  throw( std::invalid_argument(
			  "LegacyBundleSolver::put_State(): inconsistent State" ) );
 ...
 }  // end( LegacyBundleSolver::guts_of_put_State )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::FormD( void )
{
 ...
 {           // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   * continue;
   *
   * However, currently LegacyBundleSolver only admits QPPenaltyMP or OSIMPSolver
 ...
 }  // end( LegacyBundleSolver::FormD )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::UpdtCntrs( void )
{
...
void LegacyBundleSolver::FormLambda1( HpNum Tau )
{
 ...
 }  // end( LegacyBundleSolver::FormLambda1 )

/*--------------------------------------------------------------------------*/

LegacyBundleSolver::Index LegacyBundleSolver::InnerLoop( bool extrastep )
{
 ...
 }  // end( LegacyBundleSolver::InnerLoop )

/*--------------------------------------------------------------------------*/

bool LegacyBundleSolver::FiAndGi( Index wFi , bool getgi )
{
 ...
 }  // end( LegacyBundleSolver::FiAndGi )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::SetupFiStrPar( Index wFi )
{
 ...
 }  // end( LegacyBundleSolver::SetupFiStrPar )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::SetupFiLambda1( Index wFi )
{
 auto fwFi = v_c05f[ wFi ];
 ...
 }  // end( LegacyBundleSolver::SetupFiLambda1 )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::SetupFiLambda( Index wFi )
{
 auto fwFi = v_c05f[ wFi ];
 ...
 }  // end( LegacyBundleSolver::SetupFiLambda )

/*--------------------------------------------------------------------------*/

bool LegacyBundleSolver::GetGi( Index wFi )
{
 ...
 for( Index Ftchd = 0 ; Ftchd < aBP3 ; ++Ftchd ) {
  ...
  if( cp < InINF ) {  // the item is a copy - - - - - - - - - - - - - - - - -
   ...
   if( OldA1k >= Alfa1k + std::max( std::abs( Alfa1k ) , double( 1 ) )
    ...
    if( ( BPar7 & 3 ) < 3 ) {
     // LegacyBundleSolver does not immediately replace the copy unless necessary,
 ...
 }  // end( LegacyBundleSolver::GetGi )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::GotoLambda1( void )
{
 // compute DeltaFi - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  * different from the one used in LegacyBundleSolver; in particular, the total
 ...
 }  // end( LegacyBundleSolver::GotoLambda1 )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::GotoLambda( void )
{
 // compute DeltaFi - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 }  // end( LegacyBundleSolver::GotoLambda )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::ResetAlfa( Index k )
{
 ...
 }  // end( LegacyBundleSolver::ResetAlfa )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::SimpleBStrat( void )
{
 ...
 }  // end( LegacyBundleSolver::SimpleBStrat )

/*--------------------------------------------------------------------------*/

double LegacyBundleSolver::BetaK( Index wFi ) {
 return( 1.0 / double( NrFi - NrEasy ) );
 }

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::Log1( void )
{
 if( ( ! f_log ) || ( LogVerb <= 1 ) )
  return;
 ...
 *f_log << " ~ Sigma = " << Sigma << std::endl << "           ";
 ...
 }  // end( LegacyBundleSolver::Log1 )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::Log2( double ft )
{
 if( ( ! f_log ) || ( LogVerb <= 1 ) )
  return;
 ...
 *f_log << std::endl;
 ...
 }  // end( LegacyBundleSolver::Log2 )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::compute_NrmZFctr( void )
{
 auto wf = ( WZNorm << 2 );
...
bool LegacyBundleSolver::FindNext( void )
{
 Index InitwFi = f_wFi;
 do {
 ...
 }  // end( LegacyBundleSolver::FindNext )
...
HpNum LegacyBundleSolver::Heuristic( Index whch )
{
 switch( whch & 3 ) {
...
HpNum LegacyBundleSolver::Heuristic1( void )
{
...
HpNum LegacyBundleSolver::Heuristic2( void )
{
...
HpNum LegacyBundleSolver::Heuristic3( void )
{
 auto NZ2 = NrmZ * NrmZ;
...
HpNum LegacyBundleSolver::Heuristic4( void )
{
 auto NZ2 = NrmZ * NrmZ;
 if( G1Norm == INFshift )
  G1Norm = norm( G1 , 2 );
...
void LegacyBundleSolver::InitMP( void )
{
 // set the size- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 }  // end( LegacyBundleSolver::InitMP )

/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::BStrategy( Index wFi )
{
 ...
 }  // end( LegacyBundleSolver::BStrategy )

/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::FindAPlace( Index wFi )
{
 ...
 }  // end( LegacyBundleSolver::FindAPlace )

/*--------------------------------------------------------------------------*/

bool LegacyBundleSolver::NeedsAlfa1( void )
{
...
bool LegacyBundleSolver::NeedsScPr1( void )
{
...
bool LegacyBundleSolver::NeedsG1( void )
{
...
void LegacyBundleSolver::UpdateHeuristicInfo( void )
{
...
void LegacyBundleSolver::guts_of_destructor( void )
{
 if( Master ) {
  Master->SetDim();
  #if( USE_MPTESTER )
 ...
 }  // end( LegacyBundleSolver:guts_of_destructor )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::ReSetAlg( unsigned char RstLvl )
{
 ...
 }  // end( LegacyBundleSolver::ReSetAlg )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::Delete( cIndex i , bool ModDelete )
{
 ...
 }  // end( LegacyBundleSolver::Delete )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::UpdtaBP3( void )
{
 if( BPar5 == 0 )
  return;
 ...
 }  // end( LegacyBundleSolver::UpdtaBP3 )

/*--------------------------------------------------------------------------*/

bool LegacyBundleSolver::IsOptimal( double eps ) const
{
 ...
 }  // end( LegacyBundleSolver::IsOptimal )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::FModChg( VarValue shift , Index wFi )
{
 ...
 }  // end( LegacyBundleSolver::FModChg )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::remove_from_global_pool( Index k , Index i , bool hard )
{
 ...
 }  // end( LegacyBundleSolver::remove_from_global_pool )

/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::find_place_in_global_pool( Index k )
{
 ...
 }  // end( LegacyBundleSolver::find_place_in_global_pool )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::add_to_global_pool( Index k , Index i , Index wh )
{
 ...
 }  // end( LegacyBundleSolver::add_to_global_pool( k , i , wh ) )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::add_to_bundle( Index k , Index i )
{
 ...
 }  // end( LegacyBundleSolver::add_to_bundle )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::reset_bundle( void )
{
 // completely resets the bundle, because a (bunch of) Modification(s) saying
 // so has(ve) been received. this only affects the LegacyBundleSolver data
 ...
 }  // end( LegacyBundleSolver::reset_bundle )

/*--------------------------------------------------------------------------*/

Lst_sp_Mod::size_type LegacyBundleSolver::num_outstanding_Modification( void )
{
 auto res = v_mod.size();
...
bool LegacyBundleSolver::is_special_GroupMod( GroupModification & gmod )
{
 ...
 }  // end( LegacyBundleSolver::is_special_GroupMod )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::flatten_Modification_list( Lst_sp_Mod & vmt , sp_Mod mod )
{
...
void LegacyBundleSolver::flatten_easy_Modification_list( Lst_sp_Mod & vmt ,
						   sp_Mod mod )
{
...
void LegacyBundleSolver::process_outstanding_easy_Modification( void )
{
...
void LegacyBundleSolver::process_outstanding_Modification( void )
{
 ...
 for( auto rimod = v_mod_tmp.rbegin() ; rimod != v_mod_tmp.rend() ;
  ...
  if( const auto tmod = std::dynamic_pointer_cast< FunctionMod >( mod ) ) {
   ...
   if( wFi < NrFi )
    ...
    // business of LegacyBundleSolver
    to_delete = true;
    continue;
    }
  ...
  // "physical" Modification that LegacyBundleSolver does not care about
  ...
  if( cntreset ) {           // at least a (non-easy) component has been reset
   for( Index k = 0 ; k < NrFi ; ++k )
    if( reset[ k ] ) {       // reset[ k ] ==> ! IsEasy[ k ]
     // if LegacyBundleSolver "plays nice" with other Solvers, it keeps track of
     ...
     // the sense of updating the LegacyBundleSolver data structures, since they
 ...
 // version of LegacyBundleSolver that does not use MPSolver since there the
 ...
 for( auto rimod = v_mod_tmp.rbegin() ; rimod != v_mod_tmp.rend() ;
  ...
  // a specialised way by LegacyBundleSolver, but is currently not, and
 ...
 // at this point, the set of Variable in the LegacyBundleSolver/Master Problem
 ...
 // in the linearizations corresponds to what LegacyBundleSolver expects
 ...
 if( toadd ||
  ...
  for( Index k = 0 ; k < NrFi ; ++k ) {
   ...
   // ( BPar7 & 3 ) == 3, in which case LegacyBundleSolver does not care if there
 ...
 }  // end( LegacyBundleSolver::process_outstanding_Modification )

/*--------------------------------------------------------------------------*/

#ifndef NDEBUG

void LegacyBundleSolver::CheckBundle( void )
{
 std::ostream * wlog = ( ( ! f_log ) || ( LogVerb <= 1 ) ) ? & std::cerr
                                                           : f_log;
 // check vBPar2
 ...
 }  // end( LegacyBundleSolver::CheckBundle )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::CheckAlpha( void )
{
 std::ostream * wlog = ( ( ! f_log ) || ( LogVerb <= 1 ) ) ? & std::cerr
                                                           : f_log;
 *wlog << def;
 ...
 }  // end( LegacyBundleSolver::CheckAlpha )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::CheckLBs( void )
{
 std::ostream * wlog = ( ( ! f_log ) || ( LogVerb <= 1 ) ) ? & std::cerr
                                                           : f_log;
 *wlog << def;
 ...
 }  // end( LegacyBundleSolver::CheckLBs )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolver::PrintBundle( void )
{
 std::ostream * wlog = ( ( ! f_log ) || ( LogVerb <= 1 ) ) ? & std::cerr
                                                           : f_log;
...
/*--------------------------------------------------------------------------*/
/*------------ METHODS OF LegacyBundleSolver::FakeFiOracle -----------------*/
/*--------------------------------------------------------------------------*/
/*-------------- METHODS FOR READING THE DATA OF THE PROBLEM ---------------*/
/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::FakeFiOracle::GetNumVar( void ) const
{
 return( bslv->NumVar );
 }
...
Index LegacyBundleSolver::FakeFiOracle::GetNrFi( void ) const
{
 return( bslv->NrFi );
 }
...
Index LegacyBundleSolver::FakeFiOracle::GetMaxName( void ) const
{
...
bool LegacyBundleSolver::FakeFiOracle::GetUC( cIndex i )
{
...
LMNum LegacyBundleSolver::FakeFiOracle::GetUB( cIndex i )
{
 const auto var = bslv->LamVcblr[ i ];
...
Index LegacyBundleSolver::FakeFiOracle::GetBNC( cIndex wFi )
{
...
Index LegacyBundleSolver::FakeFiOracle::GetBNR( cIndex wFi )
{
...
Index LegacyBundleSolver::FakeFiOracle::GetBNZ( cIndex wFi )
{
...
void LegacyBundleSolver::FakeFiOracle::GetBDesc( cIndex wFi , int * Bbeg ,
					   ...
					   double * lbd , double * ubd )
{
 ...
 }  // end( LegacyBundleSolver::FakeFiOracle::GetBDesc )

/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::FakeFiOracle::GetANZ( cIndex wFi ,
...
void LegacyBundleSolver::FakeFiOracle::GetADesc( cIndex wFi , int * Abeg ,
					   ...
					   cIndex strt , Index stp )
{
 ...
 }  // end( LegacyBundleSolver::FakeFiOracle::GetANZ )

/*--------------------------------------------------------------------------*/
/*------------- METHODS FOR READING SUBGRADIENTS / CONSTRAINTS -------------*/
/*--------------------------------------------------------------------------*/

Index LegacyBundleSolver::FakeFiOracle::GetGi( SgRow SubG , cIndex_Set & SGBse ,
...
void LegacyBundleSolverState::deserialize( const netCDF::NcGroup & group )
{
 auto nv = group.getDim( "LegacyBundleSolver_NumVar" );
 if( nv.isNull() )
  throw( std::logic_error(
		      "LegacyBundleSolverState::deserialize: missing NumVar" ) );

 NumVar = nv.getSize();

 auto l = group.getVar( "LegacyBundleSolver_Lambda" );
 if( l.isNull() )
  throw( std::logic_error(
		      "LegacyBundleSolverState::deserialize: missing Lambda" ) );

 Lambda.resize( nv.getSize() );
 l.getVar( Lambda.data() );

 auto tt = group.getVar( "LegacyBundleSolver_t" );
 if( tt.isNull() )
  throw( std::logic_error( "LegacyBundleSolverState::deserialize: missing t" ) );

 tt.getVar( &t );

 auto nf = group.getDim( "LegacyBundleSolver_NrFi" );
 if( nf.isNull() )
  throw( std::logic_error(
		      "LegacyBundleSolverState::deserialize: missing NrFi" ) );

 NrFi = nf.getSize();
 --NrFi;
 
 auto nup = group.getDim( "LegacyBundleSolver_UpFiLmbdef" );
 if( nup.isNull() ) {
  UpFiLmbdef = 0;
  UpFiLmb.clear();
  }
  ...
  auto vup = group.getVar( "LegacyBundleSolver_UpFiLmb" );
  if( vup.isNull() )
   throw( std::logic_error(
		      "LegacyBundleSolverState::deserialize: missing UpFiLmb" ) );

  UpFiLmb.resize( NrFi + 1 );
  vup.getVar( UpFiLmb.data() );
  }

 auto nlw = group.getDim( "LegacyBundleSolver_LwFiLmbdef" );
 if( nlw.isNull() ) {
  LwFiLmbdef = 0;
  LwFiLmb.clear();
  }
  ...
  auto vup = group.getVar( "LegacyBundleSolver_LwFiLmb" );
  if( vup.isNull() )
   throw( std::logic_error(
		      "LegacyBundleSolverState::deserialize: missing LwFiLmb" ) );

  LwFiLmb.resize( NrFi + 1 );
  vup.getVar( LwFiLmb.data() );
  }

 auto f0 = group.getVar( "LegacyBundleSolver_Fi0Lmb" );
 if( f0.isNull() )
  Fi0Lmb = 0;
 else
  f0.getVar( &Fi0Lmb );

 auto lb = group.getVar( "LegacyBundleSolver_global_LB" );
 if( lb.isNull() )
  global_LB = -LegacyBundleSolver::INFshift;
 else
  lb.getVar( &global_LB );
 ...
 for( LegacyBundleSolver::Index i = 0 ; i < v_comp_State.size() ; ++i ) {
  auto gi = group.getGroup( "Component_State_" + std::to_string( i ) );
  if( ! gi.isNull() )
    v_comp_State[ i ] = State::new_State( gi );
  }
 }  // end( LegacyBundleSolverState::deserialize )

/*--------------------------------------------------------------------------*/

void LegacyBundleSolverState::serialize( netCDF::NcGroup & group ) const
{
 ...
 if( global_LB > - LegacyBundleSolver::INFshift )
  ( group.addVar( "LegacyBundleSolver_global_LB" , netCDF::NcDouble() )
    ).putVar( & global_LB );
 ...
 }  // end( LegacyBundleSolverState::serialize )

/*--------------------------------------------------------------------------*/
/*-------------------- End File LegacyBundleSolver.cpp ---------------------*/
/*--------------------------------------------------------------------------*/
